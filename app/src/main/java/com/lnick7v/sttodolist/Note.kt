...
// аннотация @field:PrimaryKey относится к val id и сообщает что столбец id является "Первичным ключем",
// т.е. теперь id должно быть уникально для каждой записи. В таблице может быть только 1 запись с определенным id,
// при попытке добавить запись с таким же id, то в зависимости от настроек поведения таблицы,
// она либо не будет добавлена, либо будет добавлена, а старая удалится. Параметр autoGenerate,
// отвечает генерировать ли первичный ключ автоматически или нет.
// Запись @field:PrimaryKey - используется при добавлении анотации в конструкторе класса, если в теле класса, то пишем @PrimaryKey
